 ...
 */
class JarVerifier {
    private static final String TAG = "JarVerifier";
    ...
    Certificate[] getCertificates(String name) {
		if (certificates.size() > 0) {
			// fred: assume only one pair: SF<-->RSA(DSA,EC)
			Enumeration<Certificate[]> e = certificates.elements();
            if (e.hasMoreElements()) {
                Certificate[] verifiedCerts = e.nextElement();
